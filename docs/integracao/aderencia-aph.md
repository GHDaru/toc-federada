...
| §B.5.2 | `app_id` estável; `action_id`/`screen.id` namespaced `toc.*` | ● atendido | `apps/api/src/toc_api/dominio/federacao/admissao.py`, `catalogo.py` e `telas.py`; [`../../scripts/check-manifesto.sh`](../../scripts/check-manifesto.sh) valida o manifesto contra o schema normativo do Anexo B — 12 telas, 20 ações, 7 sabotagens repelidas |
