...
- [x] T-07 — Produto e planejamento: `docs/produto/visao.md`, `docs/produto/modulos.md`,
  `docs/roadmap.md` (12 ciclos, portões, "não pode começar sem"). · Dep: T-05 ·
  Ref: RF-07..RF-09 · Aceite: DoD linha 6.
- [x] T-08 — ADRs 0001–0008 no formato da irmã (Princípios tocados, alternativas com
...
- [x] T-09 — Specs de módulo 002–012 (formato do brief §7: taxonomia, EARS, selos,
...
- [x] T-10 — Portões novos: `scripts/check-caminhos.sh` (R4),
...
- [x] T-11 — Site: vendorizar o gerador em `tools/product-site/` com atribuição (ADR
  0008), adaptar para RI e vocabulário TOC, gerar `docs/product-site/`. · Dep: T-07,
  T-09 · Ref: RF-12 · Aceite: DoD linha 10.
- [x] T-12 — `CHANGELOG.md` com a entrada desta fundação. · Dep: T-06..T-11 · Aceite:
  entrada em `[Unreleased]` citando o ciclo 001.

## Portões e crítica

- [x] T-13 — Rodar TODAS as aptidões (método + as três novas + DoD completa) e colar
...
- [x] T-14 — Gauntlet: críticos em contexto fresco comparam às cegas contra as três
...
- [x] TAIL:review — revisão independente em contexto fresco, por quem não construiu
- [x] TAIL:security — passe de segurança proporcional à classe de risco (aqui: vazamento
  de dado real e de segredo em texto)
- [x] TAIL:mutation — cada portão criado neste ciclo (T-10), sabotado de propósito e
  visto recusando
- [ ] TAIL:gate — DoD verde → veredito do guardião → gate humano de merge (indelegável)

> **Por que T-06 e TAIL:gate seguem em branco.** T-06 tem por aceite as linhas 1, 2 e 11
> da DoD; as duas primeiras saíram `7` e `1`, mas a **linha 11 saiu `1` onde espera `0`**
> (`qa-report.md` §4.2) — o aceite não fechou, e marcar assim mesmo seria a caixa sem
> testemunha. `TAIL:gate` é do Product Steward: quem executou não aprova o que executou.
